(() => {
  ...
  var ListSortable = class {
    constructor(table, config) {
      this.tableBody = table.querySelector("tbody");
      this.config = config;
      this.sortable = sortable_esm_default.create(this.tableBody, {
        animation: 150,
        handle: ".handle",
        draggable: "tr",
    ...
    selectActionChanged(mutationsList) {
      for (const mutation of mutationsList) {
        if (mutation.type === "attributes" && mutation.attributeName === "class") {
          const tableRow = mutation.target;
          if (tableRow.classList.contains("selected")) {
            sortable_esm_default.utils.select(tableRow);
          } else {
            sortable_esm_default.utils.deselect(tableRow);
          }
        }
      }
    }
    ...
    async onEnd(evt) {
      ...
      if (evt.items.length === 0) {
        if (evt.newIndex < evt.oldIndex) {
          ...
          lastChild2 = evt.oldIndex;
        } else if (evt.newIndex > evt.oldIndex) {
          firstChild = evt.oldIndex;
          lastChild2 = evt.newIndex;
        } else {
          return;
        }
      } else {
        ...
        evt.oldIndicies.forEach((item) => {
          ...
          lastChild2 = Math.max(lastChild2, item.index);
        });
      ...
      const updatedRows = this.tableBody.querySelectorAll(`tr:nth-child(n+${firstChild + 1}):nth-child(-n+${lastChild2 + 1})`);
    ...
    resetActions() {
      ...
      actionsEls.forEach((elem2) => {
        const tableRow = elem2.closest("tr");
        sortable_esm_default.utils.deselect(tableRow);
  ...
  var ActionForm = class {
    constructor(formElement, config) {
      ...
      this.pageInput = document.getElementById("changelist-form-page");
  ...
  var InlineSortable = class {
    constructor(inlineFieldSet) {
      ...
      if (tBody) {
        ...
        this.sortable = sortable_esm_default.create(tBody, {
          animation: 150,
        ...
        this.sortable = sortable_esm_default.create(inlineFieldSet, {
          animation: 150,
          handle: "h3",
    ...
    onEnd(evt) {
      ...
      if (this.reversed) {
        originals.forEach((element, index2) => {
          const reorderInputElement = element.querySelector("input._reorder_");
          reorderInputElement.value = `${originals.length - index2}`;
        });
      } else {
        originals.forEach((element, index2) => {
          const reorderInputElement = element.querySelector("input._reorder_");
          reorderInputElement.value = `${index2 + 1}`;
        });
      }
    }
  };
  window.addEventListener("load", (event) => {
    ...
    for (let inlineFieldSet of document.querySelectorAll("fieldset.sortable")) {
      new InlineSortable(inlineFieldSet);
    }
  });
})();
/**!
